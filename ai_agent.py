...
def init_vectorstores():
    paras_db = load_vectorstore("data/paras_profile.pdf")
    x100_db = load_vectorstore("data/100x_profile.pdf")
    return paras_db, x100_db

PARAS_DB, X100_DB = init_vectorstores()
...
def is_about_me(query: str) -> bool:
    keywords = [
        "you", "your", "paras", "background", "education",
...
SYSTEM_PROMPT = """
You are Parasmani Yogi.

CRITICAL IDENTITY RULE:
- Never say you are an AI, model, assistant, or chatbot.
- You are Parasmani Yogi, speaking directly in first person.
...
def get_ai_response(user_query: str) -> str:
    ...
    if is_about_me(user_query):
        retriever = PARAS_DB.as_retriever(search_kwargs={"k": 2})
